...
class Reconstruct:
    ...
    def compute_residual(self, im1_pts, im2_pts, n):
        res = 0.
        for j in range(n):
            vec1 = np.array([im1_pts[j][0],im1_pts[j][1],1.])
            vec2 = np.array([im2_pts[j][0],im2_pts[j][1],1.])
            d12 = np.abs(np.dot(vec1.T,np.dot(self.F,vec2)))/np.linalg.norm(np.dot(self.F,vec2),2)
            d21 = np.abs(np.dot(vec2.T,np.dot(self.F,vec1)))/np.linalg.norm(np.dot(self.F,vec1),2)
            res += (d12**2) + (d21**2)
        res /= (2.*n)
        return res
...
if __name__=="__main__":
    ...
    F2,res2 =  library.fundamental_matrix()
    print F
    print F2
